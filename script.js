...
var allCoins    = []; // все монеты с биржи
...
window.addEventListener("load", function() {
  ...
  ["Подключение...","Авторизация...","Загрузка данных..."].forEach(function(m, i) {
  ...
  if (actBar) {
    ...
    fxBtn.className = "btn-ghost"; fxBtn.style.marginTop = "0";
...
function loadCoins(exchange) {
  ...
  fetch(API_URL + "/coins/" + exchange)
    ...
    .catch(function() {
      if (cc) cc.innerHTML = '<div class="loading-coins">Ошибка загрузки. Попробуй позже.</div>';
    });
}
...
function renderCoinCards(coins) {
  ...
  cc.innerHTML = "";
  if (!coins.length) {
    cc.innerHTML = '<div class="loading-coins">Монеты не найдены</div>';
    return;
  }
