...
(function () {
  ...
  var S = 200; cv.width = cv.height = S;
  var G = 6, COLS = Math.floor(S / G);
  var YACC = "#F5C518";
...
function showScreen(id) {
  ...
  for (var i = 0; i < screens.length; i++) {
    ...
    if (s.id === id) {
      ...
      (function(el){ requestAnimationFrame(function(){ el.classList.add("active"); }); }(s));
    } else if (s.classList.contains("active")) {
      s.classList.add("out"); s.classList.remove("active");
      (function(el){ setTimeout(function(){ if (!el.classList.contains("active")) el.style.display = "none"; }, 300); }(s));
    }
  }
}
...
window.addEventListener("load", function() {
  buildChips(); buildTutDots();
  var statEl = document.getElementById("load-status");
  var msgs   = ["Подключение...","Авторизация...","Загрузка данных..."];
  for (var i = 0; i < msgs.length; i++) {
    (function(m, delay){ setTimeout(function(){ if(statEl) statEl.textContent = m; }, delay); }(msgs[i], 300 + i*650));
  }
...
function authUser() {
  fetch(API_URL + "/auth", {
    ...
    body: JSON.stringify({ id: TG_ID, username: TG_NAME, first_name: TG_FIRST, last_name: TG_LAST, language: TG_LANG, platform: TG_PLATFORM })
  })
  .then(function(res){ return res.json(); })
  .then(function(data){ data.already_registered ? showExchange() : showScreen("screen-tutorial"); })
  .catch(function(){ localStorage.getItem("cs_done") ? showExchange() : showScreen("screen-tutorial"); });
}

/* ── TUTORIAL ── */
function buildTutDots() {
  var wrap = document.getElementById("tut-dots"); if (!wrap) return;
  wrap.innerHTML = "";
...
document.getElementById("tut-next").addEventListener("click", function(){
  tutIdx < TUT_CNT - 1 ? setTutSlide(tutIdx + 1) : finishTut();
});
document.getElementById("tut-skip").addEventListener("click", finishTut);
function finishTut(){ localStorage.setItem("cs_done","1"); showExchange(); }
...
function buildChips() {
  ...
  if (ec) {
    for (var i = 0; i < EXCHANGES.length; i++) {
      var e = EXCHANGES[i], el = document.createElement("div");
      el.className = "chip"; el.dataset.id = e.id;
      el.innerHTML = '<span class="chip-icon">' + e.icon + "</span>" + e.label;
      (function(eid){ el.addEventListener("click", function(){ selectExchange(eid); }); }(e.id));
      ec.appendChild(el);
    }
  }
  ...
  if (cc) {
    for (var j = 0; j < COINS.length; j++) {
      var co = COINS[j], cel = document.createElement("div");
      cel.className = "chip"; cel.dataset.sym = co.sym;
      cel.innerHTML = '<span class="chip-icon">'+co.icon+"</span>"+co.sym+' <span style="opacity:.45;font-size:11px">'+co.name+"</span>";
      (function(sym){ cel.addEventListener("click", function(){ selectCoin(sym); }); }(co.sym));
      cc.appendChild(cel);
    }
  }
}
function selectExchange(id) {
  selExchange = id;
  document.querySelectorAll("#exchange-chips .chip").forEach(function(c){ c.classList.toggle("selected", c.dataset.id === id); });
  var btn = document.getElementById("exchange-next"); if (btn) btn.disabled = false;
}
function selectCoin(sym) {
  selCoin = sym;
  document.querySelectorAll("#coin-chips .chip").forEach(function(c){ c.classList.toggle("selected", c.dataset.sym === sym); });
  var btn = document.getElementById("coin-next"); if (btn) btn.disabled = false;
  var coin = COINS.find(function(c){ return c.sym === sym; });
  var orb  = document.getElementById("orb-core"); if (orb) orb.textContent = coin ? coin.icon : sym[0];
}
...
document.getElementById("coin-search").addEventListener("input", function(){
  var q = this.value.toLowerCase(), first = null;
...
document.getElementById("res-back").addEventListener("click", function(){ showScreen("screen-coin"); });

/* ── ALERT PCT ── */
document.getElementById("pct-minus").addEventListener("click", function(){ if (alertPct > 1)  { alertPct--; updatePct(); } });
document.getElementById("pct-plus").addEventListener("click",  function(){ if (alertPct < 50) { alertPct++; updatePct(); } });
...
function startAnimate() {
  var fill = document.getElementById("an-fill"), sub = document.getElementById("an-step");
  if (!fill || !sub) return;
  anIdx = 0; fill.style.width = "10%"; sub.textContent = AN_STEPS[0];
...
function stopAnimate() {
  ...
  var fill = document.getElementById("an-fill"); if (fill) fill.style.width = "100%";
}
...
function doAnalyze() {
  ...
  .catch(function(){ stopAnimate(); showExchange(); setTimeout(function(){ toast("Сервер не отвечает. Попробуйте позже.", "err"); }, 350); });
}
...
function renderResult(d) {
  ...
  var trend = String((fc.trend) || "").toLowerCase();
  if (dot) {
  ...
  setText("ph-name",  d.name || d.symbol || selCoin || "");
  setText("ph-price", fmtUSD(p));
  ...
  if (el24) { el24.textContent = "24h: "+(c24>=0?"+":"")+c24.toFixed(2)+"%"; el24.className = "chg-chip "+(c24>=0?"pos":"neg"); }
  if (el7)  { el7.textContent  = "7d: " +(c7 >=0?"+":"")+c7.toFixed(2)+"%";  el7.className  = "chg-chip "+(c7 >=0?"pos":"neg"); }

  var rec = String(ai.recommendation || "держать").toLowerCase();
  var rb  = document.getElementById("rec-badge");
  if (rb) {
    rb.textContent = capitalize(rec); rb.className = "rec-badge";
  ...
  setText("sentiment-tag", String(ai.sentiment || "нейтральный"));
  ...
  setText("met-vol",   String(m.volatility  || "—"));
  setText("met-liq",   String(m.liquidity   || "—"));
  setText("met-tech",  m.tech_score        ? m.tech_score        + "/100" : "—");
  setText("met-fund",  m.fundamental_score ? m.fundamental_score + "/100" : "—");

  setText("fg-7",   fc.predicted_7d  ? fmtUSD(fc.predicted_7d)  : "—");
  setText("fg-30",  fc.predicted_30d ? fmtUSD(fc.predicted_30d) : "—");
  setText("lv-sup", fc.support       ? fmtUSD(fc.support)       : "—");
  setText("lv-res", fc.resistance    ? fmtUSD(fc.resistance)    : "—");

  var conf = Number(fc.confidence) || 0;
  setText("conf-pct", conf + "%");
  setTimeout(function(){ var f = document.getElementById("conf-fill"); if (f) f.style.width = conf + "%"; }, 200);

  setText("ai-summary", String(ai.summary     || ""));
  setText("ai-risks",   String(ai.risks       || "—"));
  setText("ai-opp",     String(ai.opportunity || "—"));

  setText("monitor-text",
...
function drawChart(hist) {
  var cv = document.getElementById("price-chart"); if (!cv) return;
  if (activeChart) { activeChart.destroy(); activeChart = null; }
  if (!hist || !hist.length) return;
  var ctx = cv.getContext("2d"), prices = [], labels = [];
  for (var i = 0; i < hist.length; i++) { prices.push(Number(hist[i].price)||0); labels.push(String(hist[i].day||"")); }
  ...
  activeChart = new Chart(ctx, {
    type: "line",
    data: { labels: labels, datasets: [{ data: prices, borderColor: color, borderWidth: 2, backgroundColor: grad, fill: true, tension: 0.45, pointBackgroundColor: color, pointRadius: 3, pointHoverRadius: 6 }] },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: {
        legend: { display: false },
        tooltip: {
          backgroundColor: "#161A1F", borderColor: "rgba(245,197,24,0.25)", borderWidth: 1,
          titleColor: "#7A7060", bodyColor: color,
          bodyFont: { family:"'Space Mono'", weight:"700", size:13 }, padding: 10,
          callbacks: { label: function(c){ return " " + fmtUSD(c.raw); } }
        }
      },
      scales: {
        x: { grid: { color:"rgba(255,255,255,0.03)" }, ticks: { color:"#35302A", font:{size:10,family:"'Space Mono'"} } },
        y: { grid: { color:"rgba(255,255,255,0.03)" }, ticks: { color:"#35302A", font:{size:10,family:"'Space Mono'"}, maxTicksLimit:4, callback: function(v){ return fmtUSD(v); } } }
      }
    }
  });
}
...
document.getElementById("new-btn").addEventListener("click", function(){
  ...
  document.querySelectorAll(".chip").forEach(function(c){ c.classList.remove("selected","hidden"); });
  var en = document.getElementById("exchange-next"), cn = document.getElementById("coin-next");
  if (en) en.disabled = true; if (cn) cn.disabled = true;
  var es = document.getElementById("exchange-search"), cs = document.getElementById("coin-search");
  if (es) es.value = ""; if (cs) cs.value = "";
  updatePct(); showExchange();
});
